...
3. In Chrome go to `chrome://extensions`, turn on Developer mode, click **Load unpacked**, and choose `~/notate-extension`. After later code changes, click **Reload** on Notate. The card should say version **1.6**.
...
6. How it works: **New** → click the element that caught your eye → write why it mattered → Save (⌘↩). Opening a saved page shows your notes while the site still works. **Done** finishes adding, or puts Notate away. Drag a note to move it while adding.
